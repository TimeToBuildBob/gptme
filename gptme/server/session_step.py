...
def step(
    ...
    # A reserved continuation can be interrupted after its worker starts but
    # before it reaches step(). Bail out before hooks, log writes, or lifecycle
    # events make an aborted generation observable.
    if not session.generating:
        logger.debug(
            "step(): aborting setup — session %s was interrupted",
            session.id,
        )
        session.generating_since = None
        return
    ...
    def execute_tool_thread() -> None:
        ...
        try:
            ...
            if not session.pending_tools:
                # Reserved workers inherit an existing generation reservation;
                # an interrupt invalidates it. Unreserved workers must atomically
                # acquire a new reservation because step() clears generating
                # after launching the tool worker.
